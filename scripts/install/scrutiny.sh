...
_install () {
    ...
    if [[ $(arch) =~ "_64" ]]; then 
        ...
        echo_error "Arch not currently supported, exiting"
        exit 1
    fi

    echo_progress_start "Downloading binaries and extracting source code" | tee -a $log
    ...
    if [[ $? != 0 ]]; then
        echo_error "Failed to query github" | tee -a $log
        exit 1
    fi
    ...
    echo_progress_done "Downloaded and extracted"
