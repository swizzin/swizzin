...
for u in "${users[@]}"; do
    ...
    if [ -e /etc/sudoers.d/$u ]; then
        ...
        : #The file does not exist and it should be handled differently ()this happens to me all the time
    fi
done
...
if [ -e /install/.panel.lock ]; then

    if [[ -f /etc/sudoers.d/panel ]]; then
        echo_info "Fixing panel sudoers"
