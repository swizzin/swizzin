...
if [[ -f /install/.bazarr.lock ]]; then
	...
	if ! grep -q .venv /etc/systemd/system/bazarr.service; then
		...
		if ! sudo -u ${user} bash -c "/opt/.venv/bazarr/bin/pip3 install -r requirements.txt" > /dev/null 2>&1; then
	...
	if ! grep -q numpy <(/opt/.venv/bazarr/bin/pip freeze); then
		if ! sudo -u ${user} bash -c "/opt/.venv/bazarr/bin/pip3 install -r /opt/bazarr/requirements.txt" > /dev/null 2>&1; then
			echo_error "Python requirements failed to install inside the venv"
			exit 1
		fi
		systemctl try-restart bazarr
