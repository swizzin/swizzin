...
if [[ -f /etc/apt/sources.list.d/mono-xamarin.list ]]; then
  if grep -q snapshot /etc/apt/sources.list.d/mono-xamarin.list; then
    :
  else
    echo "deb https://download.mono-project.com/repo/debian wheezy/snapshots/4.8/. main" > /etc/apt/sources.list.d/mono-xamarin.list
    echo "Downgrading to mono 4.8 snapshot"
    version=wheezy
  ...
  echo "Upgrading to mono snapshot v4.8"
  echo "deb https://download.mono-project.com/repo/debian wheezy/snapshots/4.8/. main" > /etc/apt/sources.list.d/mono-xamarin.list
  apt-get -y -q update
